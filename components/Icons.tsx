import React from 'react';
import * as LucideIcons from 'lucide-react';

export const {
  Inbox, Send, File, Trash2, Tag, PieChart, Users, Settings,
  Search, SlidersHorizontal, Download, ChevronDown, CheckSquare,
  AlertCircle, Clock, Paperclip, Star, MoreVertical, X, Calendar,
  Briefcase, TrendingUp, Filter, Plus, FileText, ArrowUpRight,
  ArrowDownRight, Mail, LayoutDashboard, ListFilter, GripVertical, Save,
  Kanban, ListTodo, CheckCircle2, Circle, MoreHorizontal, Activity, Zap,
  ChevronLeft, ChevronRight, Menu, MessageSquare, ExternalLink,
  PenSquare, Minimize2, Maximize2, MoveDiagonal
} = LucideIcons;
